...
ERROR_PROTO_PATH = "common/error.proto"
...
def render_cats(files: list[ProtoFile]) -> str:
    ...
    lines = [
        "---@meta",
        "",
        "-- Code generated by gen_lua.py. DO NOT EDIT.",
        "",
    ]
    for proto_file in files:
...
def opcode_entries(
    ...
    for enum_name, number in find_opcode_enum(files).values:
        ...
        if name.endswith("Begin") and name not in messages:
            continue
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="从 protobuf descriptor set 生成 Lua 类型和查找表。"
    )
    parser.add_argument(
        "--schema",
        required=True,
        type=Path,
        help="输入的 protobuf descriptor set 文件路径。",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
...
if __name__ == "__main__":
    ...
    generate(arguments.schema, arguments.output_dir)
